    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant){
        ...
        if (variant.available) {
          ...
          $(this.selectors.addToCartText).text('Update item');
...
theme.quickajaxCart = function(){
    ...
    function prodAddToCart(){
	  if (theme.ajax_cart){
        ...
        $(".quickupdate-cart").click(function(i) {
			if (i.preventDefault(), "disabled" != $(this).attr("disabled"))
				/** var att = $(this).closest("form").find("select[name=id]").val();
					att || (att = $(this).closest("form").find("input[name=id]").val());
				var qty = $(this).closest("form").find("input[name=quantity]").val();
              	...
              	var id = null, 
                    qty=3, 
                    properties={};
				updateCartItemById(att, qty, properties);
			return;
		  });
	  }
    }
    function addinToCart(att, qty){
  	...
  	function updateCartItemById(id, quantity, properties) {
  		/** $("body").addClass("loading"); */
       	/** CartJS.updateItemById(id, quantity, properties = {}, options = {}) */
      	const qty = $('[id^="'+ 'updates_' + window.currentCartItemId +'"]').val() || 1
      	$.magnificPopup.close()
        CartJS.updateItemById(window.currentCartItemId, 0, {},
        {
          success: function(data, textStatus, jqXHR){
...
function refreshCartPage(){
  $.ajax({
    ...
    success(data) {
        $('form.cart div:first').html($("form.cart div:first", data).html());
        $('form.cart .cart-pay-wrap').html($("form.cart .cart-pay-wrap", data).html());
        $('.cartSummery').html($(".cartSummery", data).html());
    },
...
$(document).ready(function() {
  ...
  sections.register('quickupdate', theme.Product);
